...
export const ChangeBooking = () => {
  ...
  const [bookingForm, setBookingForm] = useState<IBookingForm>({
    date: new Date(), noPeople: 0, time: 0,});
  ...
  const editedFormattedDate = `${
    editedRedoDate[1]
  } ${+editedRedoDate[2]} ${+editedRedoDate[3]}`;
    ...
    <ChangeBookingWrapper>
      ...
      {editedReservation._id !== "" ? (
        <BookingContainer>
          <p style={{ textTransform: "capitalize", fontSize: "1.2rem" }}>
            <strong>{editedReservation.name}</strong>
          </p>
          <p>
            {editedFormattedDate}, {editedReservation.time + ":00"}
          </p>
          <p>
            <strong>{editedReservation.seats + " personer"}</strong>
          </p>
        </BookingContainer>
      ) : (
        <BookingContainer>
      ...
      <InputContainer>
        ...
        {responseMessage.message !== "" ? (
          <MessageDiv>{responseMessage.message}</MessageDiv>
        ) : null}
