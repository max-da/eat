...
export const BookingFormComponent = (props: Iprops) => {
  ...
  function sendPost() {
    if (form.email.includes("@") === false) {
      return props.errorFunc(
        true,
        "Det verkar som att du angett en ogiltig email-adress."
      );
    }
    axios.post("http://localhost:8000/bookings/", form)
      .then((res) => {
        props.errorFunc(false, "");
        setAnima(true);
        setTimeout(() => {
        ...
        <FormWindow>
          ...
          <FormInputDiv>
            <InputSpanBorder>
              ...
              <span style={{paddingLeft: "10px"}}>{props.bookingForm.date}</span>
            </InputSpanBorder>

            <InputSpanBorder>
              <FormLabel>TID</FormLabel>
              <span style={{paddingLeft: "10px"}}>{props.bookingForm.time}</span>
            </InputSpanBorder>

            <InputSpanBorder>
              <FormLabel>ANTAL GÄSTER</FormLabel>
              <span style={{paddingLeft: "10px"}}>{props.bookingForm.noPeople}</span>
            </InputSpanBorder>
